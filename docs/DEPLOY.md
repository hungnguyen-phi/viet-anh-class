...
| `NEXT_PUBLIC_SITE_URL` | public, **build-time**, tuỳ chọn | GitHub **Variable** → build-arg | Tên miền công khai của app. Bỏ trống thì lấy mặc định trong `lib/site.ts` (`https://class.truongvietanh.com`). Đặt biến này khi đổi tên miền — hoặc khi phải lùi gấp về tên miền cũ — để không phải sửa code. Nội tuyến lúc build → **đổi xong phải build lại**. Xem `docs/DOI_TEN_MIEN.md` |
