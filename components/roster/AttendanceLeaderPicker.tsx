  ...
  return (
    <div className="glass rounded-[16px] p-3.5">
      <div className="flex flex-wrap items-center gap-2">
        <span className="inline-flex items-center gap-1.5 text-[11px] font-extrabold uppercase tracking-[0.04em] text-grey-mid">
          <Star size={13} strokeWidth={2.5} className="text-gold-deep" />
          {t('attendanceLeader')}
        </span>
        <span className="text-[13.5px] font-extrabold text-navy">
        ...
        <span className="ml-auto flex items-center gap-1.5">
          {current && (
            <button
              ...
              onClick={() => save(null)}
              disabled={busy !== null}
              className="inline-flex h-8 cursor-pointer items-center gap-1.5 rounded-[9px] border-[1.5px] border-status-bad/30 bg-status-bad/[0.08] px-2.5 text-[11.5px] font-extrabold text-status-bad transition-all hover:bg-status-bad/[0.16] disabled:opacity-50"
            >
          ...
          <button
            type="button"
            onClick={() => setOpen((v) => !v)}
            className="inline-flex h-8 cursor-pointer items-center gap-1.5 rounded-[9px] border-[1.5px] border-navy/20 bg-white/60 px-2.5 text-[11.5px] font-extrabold text-navy transition-colors hover:border-navy"
          >
      ...
      {open && (
        <div className="mt-2.5 rounded-[12px] border-[1.5px] border-navy/12 bg-white/70 p-2">
          <label className="flex items-center gap-2 rounded-[10px] border-[1.5px] border-navy/15 bg-white px-2.5 focus-within:border-navy">
            ...
            <input
              ...
              className="h-9 min-w-0 flex-1 bg-transparent text-[13px] font-semibold text-navy outline-none"
            />
          </label>

          {matches.length === 0 ? (
            <p className="px-1 py-2.5 text-center text-[12.5px] font-semibold italic text-grey-mid">
              {t('leaderNoMatch')}
            </p>
          ) : (
              ...
              {matches.map((s) => {
                ...
                return (
                  <button
                    ...
                    className="flex cursor-pointer items-center gap-2 rounded-[9px] px-2 py-1.5 text-left transition-colors hover:bg-navy/[0.06] disabled:cursor-default disabled:opacity-60"
                  >
                    ...
                    <span className="min-w-0 flex-1">
                      <span className="block truncate text-[13px] font-extrabold text-navy">{s.name}</span>
                      {s.email && (
