import {CONDUCTS, CONDUCT_LABEL, CONDUCT_TEXT, type Conduct} from '@/components/grades/labels';
...
export function ClassOverview({
  ...
  return (
    <section>
      <h2 className="mb-3 font-display text-[17px] font-bold text-navy">Hạnh kiểm cả lớp</h2>
      <div className="grid grid-cols-2 gap-3.5 sm:grid-cols-4">
        {CONDUCTS.map((c) => (
          <div key={c} className="glass glass-hover rounded-[20px] p-4 text-center">
            ...
            <div className="mt-1 text-xs font-extrabold text-txt">{CONDUCT_LABEL[c]}</div>
          </div>
        ))}
      </div>
