    ...
    <>
      ...
      {mo && (
        <Popup title={nhan.tieuDe} onClose={() => setMo(false)} width="max-w-[440px]">
          {lopKhac.length === 0 ? (
            <p className="text-[13px] font-semibold text-grey-mid">{nhan.khongCoLop}</p>
          ) : (
            <XacNhanForm
              action={formAction}
            ...
            >
              ...
              <label className="flex flex-col gap-1">
                <span className="text-[10.5px] font-extrabold uppercase tracking-wide text-grey-mid">{nhan.lopNguon}</span>
