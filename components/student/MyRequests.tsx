...
export async function MyRequests({
  ...
  return (
    <section className="glass mt-3 rounded-[20px] border border-gold/40 p-[18px]">
      ...
      <p className="mt-1 text-chu-thich font-semibold text-grey-mid">{t('requestPendingHint')}</p>
          ...
          <div key={r.id} className="rounded-[12px] border-[1.5px] border-navy/10 bg-white/50 p-2.5">
            ...
            <form action={updateEditRequest} className="mt-2 flex flex-wrap items-end gap-2">
              ...
              <textarea
                ...
                className="min-h-[52px] min-w-0 flex-1 resize-y rounded-[12px] border-[1.5px] border-navy/15 bg-white px-3 py-2 text-than font-semibold text-navy outline-none focus:border-navy"
              />
              <SubmitButton className="btn-gold h-10 shrink-0 cursor-pointer rounded-[12px] px-4 text-than font-extrabold">
                {t('requestSend')}
              </SubmitButton>
            </form>
            <form action={withdrawEditRequest} className="mt-2">
              ...
              <ConfirmButton
                ...
                className="cursor-pointer rounded-[8px] border-[1.5px] border-[rgba(192,57,43,0.3)] bg-[rgba(192,57,43,0.12)] px-2.5 py-1.5 text-chu-thich font-extrabold text-status-bad transition-all hover:bg-[rgba(192,57,43,0.2)] active:translate-y-px"
              >
                {t('requestWithdraw')}
              </ConfirmButton>
            </form>
