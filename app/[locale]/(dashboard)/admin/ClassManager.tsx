  ...
  return (
    <div>
      ...
      <ul className="flex flex-col gap-2 sm:hidden">
        {shown.map((c) => (
          <li key={c.id} className="rounded-[12px] border-[1.5px] border-navy/10 bg-white p-3">
            <div className="flex items-start justify-between gap-2">
              <div className="min-w-0">
                <div className="truncate text-noi-dung font-extrabold text-navy">{c.name}</div>
                <div className="mt-0.5 text-chu-thich font-semibold text-grey-mid">
                  {c.grade ?? t('noGrade')} · {c.school_year}
                </div>
                <div className="mt-0.5 truncate text-chu-thich font-semibold text-grey-mid">
